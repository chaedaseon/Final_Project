...
public class MainPageDAO
{
	private Connection conn;
		...
		public ArrayList<BoardStudyGroupDTO> latestStudyGroupList() throws SQLException
		{
			...
			String sql = "SELECT X.NO,X.GRCODE, X.GUCODE, X.GRDATE,X.GACODE, X.PERIODCODE, X.ENDDATE";
			sql += ", X.GRLEADER, X.GRNAME, X.GRCOUNT, X.GRSUBJECT, X.GRCOMMENT, X.AGE, X.GENDER, X.LSCODE, X.CATEGORY, X.GROPEN";
			sql += ", (TRUNC(X.GRDATE + 7) - TRUNC(SYSDATE)) AS DDAY, X.GPPW, X.ENDDATE";
			sql += ", NVL((SELECT COUNT(*) FROM GROUP_JOIN WHERE GR_CODE = X.GRCODE),0) AS GJCOUNT";
			sql += " FROM(SELECT ROWNUM, ROW_NUMBER() OVER(ORDER BY ROWNUM) AS NO,T.GACODE, T.PERIODCODE, T.ENDDATE";
			...
			sql += " FROM(SELECT GACODE, PERIODCODE, GRCODE, GUCODE, GRLEADER, GRDATE, GRNAME, GRCOUNT, GRSUBJECT, GRCOMMENT, AGE, GENDER, LSCODE, CATEGORY, GROPEN, GPPW, GRDATE+7 AS ENDDATE";
			sql += " FROM GR_ALL_VIEW ORDER BY TO_NUMBER(GRCODE))T";
			sql += " WHERE (SELECT COUNT(*) FROM GR_ALL_VIEW) >= ROWNUM ";
			sql += " AND TO_CHAR(ENDDATE, 'YYYY-MM-DD') >= TO_CHAR(SYSDATE, 'YYYY-MM-DD')";
			sql += " ORDER BY ROWNUM DESC)X WHERE X.NO >= (SELECT COUNT(*)- 7 FROM GR_ALL_VIEW";
			sql += " WHERE TO_CHAR(GRDATE+7,'YYYY-MM-DD') >= TO_CHAR(SYSDATE, 'YYYY-MM-DD'))";
