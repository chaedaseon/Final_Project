...
import com.study.mybatis.model.IAdminReportAllBBSDAO;
import com.study.mybatis.model.PaginationDTO;
import com.study.mybatis.model.ReportBBSDTO;

@Controller
public class AdminReportAllBBSController
{
	@Autowired
	private SqlSession sqlSession;
	

	// 전체 게시물 신고 리스트 조회 (게시물&댓글&대댓글)
	@RequestMapping(value="adminreportallbbslist.do", method=RequestMethod.GET)
			...
			@RequestParam(value = "searchType", required = false, defaultValue = "1") String tmpsearchType) 
	{
		...
		{
		IAdminReportAllBBSDAO dao = sqlSession.getMapper(IAdminReportAllBBSDAO.class);
		...
		mav.setViewName("/WEB-INF/view/admin/AdminReportAllBBSList.jsp");
		return mav;			
		}
	}
	...
	// 게시물 신고 상세 조회
	@RequestMapping(value="adminreportallbbscontent.do", method=RequestMethod.GET)
	public String reportContent(Model model,String rtype, String code, HttpServletRequest request)
	{
		...
		if (rtype.equals("2"))
		{
			boardCode = dao.replyBoardContent(code);
		...
		{
			boardCode = dao.rereplyBoardContent(code);
			boardTitle = dao.boardTitle(boardCode);
			model.addAttribute("boardCode", boardCode);
			model.addAttribute("boardTitle", boardTitle);
		...
		return "/WEB-INF/view/admin/AdminReportAllBBSContent.jsp";
	}

	// 게시물 신고 처리
	@RequestMapping(value="adminreportallbbsprocess.do", method=RequestMethod.POST)
	public String reportProcess(Model model, HttpServletRequest request)
	{
		...
		String reported = request.getParameter("reported");			// 신고당한 사람의 게스트코드 
			
		System.out.println("신고타입"+rtype);
		System.out.println("신고번호"+code);
		System.out.println("신고유저"+rpUser);
		System.out.println("승인반려"+redCode);
		System.out.println("허위신고" + falseReport);
		System.out.println("신고당한 사람의  guCode" + reported);
		
		// 1. 게시물 / 덧글 / 대댓글 신고로 나눌 것
		// 2. 반려의 경우 허위신고 유무 체크할 것 
		//--> 허위신고 체크로 신고자에게 패널티 주기는 현재로서 불가 (신고유저, 승인반려 불필요...)
